...
async def virusscan(event):
    ...
    try:
      ...
      fsize = c.file.size
      if not fsize <= 3670000: # MAX = 3.5MB
         await gg.edit("File size exceeds 3.5MB")
         return
      api_response = api_instance.scan_file_advanced(input_file=c.file.name, allow_executables=allow_executables, allow_invalid_files=allow_invalid_files, allow_scripts=allow_scripts, allow_password_protected_files=allow_password_protected_files)
