...
async def slogo(event):
    ...
    fillcolor = "white"
    shadowcolor = "black"
    font = ImageFont.truetype("./julia/resources/fonts/ClassicCobra3D-pYGZ.otf", 400)
    ...
    draw.text((x, y), text, font=font, fill="red", stroke_width=20, stroke_fill="black")
...
async def slogo(event):
    ...
    fillcolor = "white"
    shadowcolor = "black"
    font = ImageFont.truetype("./julia/resources/fonts/Crosshatcher D.otf", 400)
    w, h = draw.textsize(text, font=font)
    h += int(h*0.21)
    image_width, image_height = img.size
    draw.text(((image_widthz-w)/2, (image_heightz-h)/2), text, font=font, fill=(255, 255, 255))
